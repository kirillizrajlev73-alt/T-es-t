...
do
    ...
    do
        ...
        crystalHubOverlay = Instance.new("TextButton")
        local overlay = crystalHubOverlay
    ...
    do
        ...
        local function findWindowFrame()
            ...
            for _, obj in ipairs(screen:GetChildren()) do
                if obj:IsA('Frame') and obj.ClipsDescendants then
                    local x = obj.AbsoluteSize.X
                    local y = obj.AbsoluteSize.Y
                    local area = x * y

                    if x >= 450 and y >= 300 and area > bestArea then
                        best = obj
                        bestArea = area
                    end
                end
        ...
        local function getWindowSize()
            local window = v300 and v300._window
            if window and window.Size then
                return window.Size.X.Offset, window.Size.Y.Offset
            end
        ...
        local function applyWindowSize(width, height)
            local size = UDim2.fromOffset(math.floor(width), math.floor(height))
            local window = v300 and v300._window
        ...
        local function getOverlay()
            return crystalHubOverlay
        end

        local function setWindowWidth(value)
            local _, height = getWindowSize()
            applyWindowSize(value, height)
        end

        local function setWindowHeight(value)
            local width, _ = getWindowSize()
            applyWindowSize(width, value)
        end
