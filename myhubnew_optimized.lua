...
do
    ...
    v301._left:Button({
        Title   = 'Wallbang Stand-Off Slider',
        Description = 'Distance behind target for force origin',
        Callback = function()
            v25('Stand-Off', 5, 50, 15, 5, function(val)
                getgenv().WALLBANG.setStandOff(val)
            end, function()
                getgenv().WALLBANG.setStandOff(15)
                v18:Notify({ Title = 'CrystalHub', Content = 'Stand-Off reset to 15', Duration = 3, Icon = 'bell' })
            end)
        end,
    })
end
