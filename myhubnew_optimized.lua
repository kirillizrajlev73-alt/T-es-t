...
do
    ...
    -- ── UI ──────────────────────────────────────────────────────────────
    VisualsTab._right:Paragraph({ Title = "Custom ESP Weapon" })
    VisualsTab._right:Toggle({
        Flag = "custom_esp_weapon",
        Title = "Weapon",
        Default = true,
        Callback = function(value)
            getgenv().CrystalCustomESPWeapon = value
        end,
    })
...
-- ============================================================
-- CUSTOM ESP imported from eclipse.lua
-- ============================================================
getgenv().CrystalCustomESPWeapon = true

-- Compatibility fallback: some executors do not expose cloneref.
local _crystal_cloneref = cloneref or function(instance)
    return instance
end

local Workspace, RunService, Players, CoreGui, Lighting = _crystal_cloneref(game:GetService("Workspace")), _crystal_cloneref(game:GetService("RunService")), _crystal_cloneref(game:GetService("Players")), game:GetService("CoreGui"), _crystal_cloneref(game:GetService("Lighting"))
...
do -- Initalize
                ...
                if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if OnScreen and Dist <= ESP.MaxDistance then
                        ...
                        if ESP.TeamCheck and plr ~= lplayer and ((lplayer.Team ~= plr.Team and plr.Team) or (not lplayer.Team and not plr.Team)) and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") and plr.Character:FindFirstChild("Humanoid") then
                            ...
                            do -- Weapons
                                Weapon.Text = "none"
                                Weapon.Visible = ESP.Drawing.Weapons.Enabled and getgenv().CrystalCustomESPWeapon ~= false
                            end                            
                        else
                            HideESP();
                        end
    ...
    do -- Update ESP
        ...
        -- CrystalHub: Custom ESP controls in the second column
        if v302 and v302._right then
            v302._right:Paragraph({Title = "Custom ESP"})
            v302._right:Toggle({
                Flag = "custom_esp_enabled",
