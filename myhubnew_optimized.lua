...
do
    ...
    -- GUI Settings: watermark and window controls
    do
        local guiSettings = v300:Tab({
            Title = 'GUI Settings',
            Icon = 'settings',
        })
        local guiLeft = guiSettings._tab:AddSection({Name = 'WATERMARK', Position = 'left'})
        local guiRight = guiSettings._tab:AddSection({Name = 'GUI', Position = 'right'})

        local function addToggle(section, title, default, callback)
            return section:AddLabel(title):AddToggle({Default = default, Callback = callback})
        end
        local function addSlider(section, title, min, max, default, callback)
            return section:AddLabel(title):AddSlider({Min = min, Max = max, Default = default, Rounding = 0, Callback = callback})
        end

        local watermarkEnabled = true
        local showFPS, showPing, showMemory, showTime = true, true, true, true
        local watermarkScale = 100

        local function setLabelVisible(label, enabled)
            if label and label.Parent then label.Visible = enabled end
        end

        addToggle(guiLeft, 'Watermark enabled', true, function(v)
            watermarkEnabled = v
            local overlayGui = game:GetService('CoreGui'):FindFirstChild('CrystalHubOpenButton')
            if overlayGui then overlayGui.Enabled = v end
        end)
        addToggle(guiLeft, 'Show FPS', true, function(v)
            showFPS = v
            local o = game:GetService('CoreGui'):FindFirstChild('CrystalHubOpenButton')
            local l = o and o:FindFirstChild('CrystalHubOverlay') and o.CrystalHubOverlay:FindFirstChild('FPS')
            if l then l.Visible = v end
        end)
        addToggle(guiLeft, 'Show Ping', true, function(v)
            showPing = v
            local o = game:GetService('CoreGui'):FindFirstChild('CrystalHubOpenButton')
            local l = o and o:FindFirstChild('CrystalHubOverlay') and o.CrystalHubOverlay:FindFirstChild('Ping')
            if l then l.Visible = v end
        end)
        addToggle(guiLeft, 'Show Memory', true, function(v)
            showMemory = v
            local o = game:GetService('CoreGui'):FindFirstChild('CrystalHubOpenButton')
            local l = o and o:FindFirstChild('CrystalHubOverlay') and o.CrystalHubOverlay:FindFirstChild('Memory')
            if l then l.Visible = v end
        end)
        addToggle(guiLeft, 'Show Time', true, function(v)
            showTime = v
            local o = game:GetService('CoreGui'):FindFirstChild('CrystalHubOpenButton')
            local l = o and o:FindFirstChild('CrystalHubOverlay') and o.CrystalHubOverlay:FindFirstChild('Time')
            if l then l.Visible = v end
        end)
        addSlider(guiLeft, 'Watermark scale', 70, 140, 100, function(v)
            watermarkScale = v
            local overlayGui = game:GetService('CoreGui'):FindFirstChild('CrystalHubOpenButton')
            local overlay = overlayGui and overlayGui:FindFirstChild('CrystalHubOverlay')
            if overlay then
                overlay.Size = UDim2.new(0, math.floor(530 * v / 100), 0, math.floor(42 * v / 100))
            end
        end)
        ...
        local function findWindowGui()
            ...
            for _, obj in ipairs(parent:GetDescendants()) do
                if obj:IsA('Frame') and obj.Visible and obj.AbsoluteSize.X > 400 and obj.AbsoluteSize.Y > 250 then
                    return obj
                end
            end
        end
        addSlider(guiRight, 'GUI width', 450, 1100, 700, function(v)
            local frame = findWindowGui()
            if frame then frame.Size = UDim2.fromOffset(v, frame.AbsoluteSize.Y) end
        end)
        addSlider(guiRight, 'GUI height', 350, 850, 550, function(v)
            local frame = findWindowGui()
            if frame then frame.Size = UDim2.fromOffset(frame.AbsoluteSize.X, v) end
        end)
        guiRight:AddButton({Name = 'Reset GUI size', Callback = function()
            local frame = findWindowGui()
            if frame then frame.Size = UDim2.fromOffset(700, 550) end
        end})

        -- Apply visibility settings to the already-created watermark labels.
        task.defer(function()
            local overlayGui = game:GetService('CoreGui'):FindFirstChild('CrystalHubOpenButton')
            local overlay = overlayGui and overlayGui:FindFirstChild('CrystalHubOverlay')
            if overlay then
                local map = {FPS = showFPS, Ping = showPing, Memory = showMemory, Time = showTime}
                for name, enabled in pairs(map) do
                    local label = overlay:FindFirstChild(name)
                    if label then label.Visible = enabled end
                end
            end
        end)
    end
