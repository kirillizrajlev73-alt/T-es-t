...
local Fatality = loadstring(game:HttpGet(
    "https://raw.githubusercontent.com/kirillizrajlev73-alt/Fatality_ui_mypaste/refs/heads/main/Fatality.lua"
))()

getgenv().CrystalHubFatality = Fatality
...
function v18:Notify(cfg)
    cfg = cfg or {}
    local ok, notifier = pcall(function()
        return Fatality:CreateNotifier()
    end)
...
local function makeControlAdapter(section)
    ...
    function api:Paragraph(cfg)
        cfg = cfg or {}
        local label = tostring(cfg.Title or "")
    ...
    function api:Dropdown(cfg)
        ...
        function control:Refresh(values)
            if self.SetData then self:SetData(values or {}) end
        end

        function control:Select(value)
            if self.SetValue then self:SetValue(value) end
        end
...
function v18:CreateWindow(cfg)
    ...
    local fatal = Fatality.new({
        Name = cfg.Title or "CrystalHub",
        Keybind = cfg.Keybind or "Insert",
        Scale = cfg.Size or UDim2.fromOffset(700, 550),
        Expire = cfg.Expire or "never",
    })

    local adapter = {_window = fatal, _tabs = {}}

    function adapter:Section(_) return self end
    ...
    function adapter:Tab(cfg2)
        ...
        local left = makeControlAdapter(menu:AddSection({
            Name = names[1], Position = "left",
        }))
        local right = makeControlAdapter(menu:AddSection({
            Name = names[2], Position = "right",
        }))

        local columns = {
            _menu = menu, _left = left, _right = right, _counter = 0,
        }
        ...
        function columns:Toggle(c) return nextCol(self):Toggle(c) end
        function columns:Button(c) return nextCol(self):Button(c) end
        function columns:Dropdown(c) return nextCol(self):Dropdown(c) end
        function columns:Slider(c) return nextCol(self):Slider(c) end
        function columns:ColorPicker(c) return nextCol(self):ColorPicker(c) end
        function columns:Keybind(c) return nextCol(self):Keybind(c) end
        function columns:Paragraph(c) return nextCol(self):Paragraph(c) end
        function columns:Divider() left:Divider(); right:Divider() end

        self._tabs[#self._tabs + 1] = columns
        return columns
    end

    function adapter:ToggleInterface()
        fatal:SetVisible(not fatal.Toggle)
    end

    function adapter:AddConfig() return fatal:AddConfig() end
    function adapter:GetFlags() return fatal:GetFlags() end
